...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="deps/smpl/smpl_models/")
...
def smpl2bvh(model_path:str, poses:str, output:str, mirror:bool,
    ...
    if poses.endswith(".npz"):
        ...
        print("try to load pkl file")
        with open(poses, "rb") as f:
            print("load pkl file")
            poses = joblib.load(f)
            rots = poses["smpl_pose"] # (N, 72)
...
if __name__ == "__main__":
    ...
    if pose_file.endswith('.pkl'):
        ...
        filelist = os.listdir(pose_file)
        for file in filelist:
            if file.endswith('.pkl'):
                file_path = os.path.join(pose_file, file)
                out_path = os.path.join(args.output, file)
                smpl2bvh(model_path=args.model_path, model_type=args.model_type, 
             mirror = args.mirror, gender=args.gender,
             poses=file_path, num_betas=args.num_betas, 
             fps=args.fps, output=out_path)
    print("finished!")