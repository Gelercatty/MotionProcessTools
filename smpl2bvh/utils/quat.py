...
# def from_axis_angle(rots, eps=1e-8):
...
#     return from_angle_axis(angle, axis)
def from_axis_angle(rots, eps=1e-8):
    angle = np.linalg.norm(rots, axis=-1, keepdims=True)  # (...,1)
    
    axis = np.zeros_like(rots)

    safe_mask = angle.squeeze(-1) > eps  # 有效值掩码
    axis[safe_mask] = rots[safe_mask] / angle[safe_mask]

    # angle恢复shape
    angle = angle.squeeze(-1)

    # 若后续的函数from_angle_axis需要axis形状为(...,3)，angle为(...)
    # 此时的axis已处理好，即便angle为0时axis也为默认0向量，不会NaN
    return from_angle_axis(angle, axis)
